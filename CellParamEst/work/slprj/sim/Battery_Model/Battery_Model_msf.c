...
return ; if ( GlobalPrm_0 != NULL ) { ( void ) memcpy ( & ( rtP__Battery2f2Battery_1Pack_1RC_sldd_ ) , GlobalPrm_0 , sizeof ( struct_Cy8NVN8uTaBKVtp81w17x ) ) ; } if ( ! ssGetModelRefGlobalParamData ( S , 1 , ( void * * ) ( & GlobalPrm_1 ) ) ) return ; if ( GlobalPrm_1 != NULL ) { ( void ) memcpy ( & ( rtP__TempInit2f2DD_Battery_sldd_ ) , GlobalPrm_1 , sizeof ( real_T ) ) ; } }
#endif
...
mdlPeriodicOutputUpdate ) ; ssSetSimulinkVersionGeneratedIn ( S , "24.2" ) ;
ssSetNeedAbsoluteTime ( S , 1 ) ; } static void mdlInitializeSampleTimes ( SimStruct * S ) { ssSetSampleTime ( S , 0 , 0 ) ; ssSetOffsetTime ( S , 0 , 0 ) ; ssSetSampleTime ( S , 1 , 0.1 ) ; ssSetOffsetTime ( S , 1 , 0 ) ; ssSetSampleTime ( S , 2 , mxGetInf ( ) ) ; ssSetOffsetTime ( S , 2 , 0 ) ; return ; }
...
storedX ) ; } { mxArray * mdlrefDW = mr_Battery_Model_GetDWork ( ) ;
mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } { mxArray * data =
mr_Battery_Model_GetSimStateDisallowedBlocks ( ) ; mxSetFieldByNumber ( ss ,
...
, & rawData [ 0 ] , numBytes ) ; } mr_Battery_Model_SetDWork ( mxGetFieldByNumber ( ss , 0 , 1 ) ) ; ssSetTNext ( S , ( time_T ) mxGetScalar ( mxGetFieldByNumber ( ss , 0 , 3 ) ) ) ; ssSetTNextTid ( S , ( int_T ) mxGetScalar ( mxGetFieldByNumber ( ss , 0 , 4 ) ) ) ; }
